...
class OperatorServiceImplTest {
    ...
    @InjectMocks
    OperatorServiceImpl serviceopM;
    @Autowired
    OperatorRepository repoOp;
    @Mock
    OperatorRepository repoOpM;

    Mock mockMvc;

   /* List<Operator> listOp = new ArrayList<Operator>(){
    OperatorRepository repoOpMockito= Mockito.mock(OperatorRepository.class);
   Operator operator = new Operator("f1","L1","aa",[new Invoice]);
        add (new Operator )
   }*/
    Operator operator;
    /*@BeforeEach
    ...
    void retrieveAllOperators() {
             ...
             Mockito.when(repoOpM.findAll()).thenReturn(operatorList);
            OperatorServiceImpl operatorService = new OperatorServiceImpl(repoOpM);
             List<Operator> result = serviceopM.retrieveAllOperators();
      ...
      /* Operator operatorToDelete = new Operator();
        ...
        assertFalse(deletedOperator.isPresent());*/
           Operator operator = new Operator();
            serviceopM.deleteOperator(operator.getIdOperateur());
    ...
    void updateOperator() {
        Operator operator = new Operator();
        when(repoOpM.save(operator)).thenReturn(operator);
...
*/
        ...
        Operator operator = new Operator();
        when(repoOpM.findById(operator.getIdOperateur())).thenReturn(Optional.of(operator));
        assertEquals(operator.getIdOperateur(),
        serviceopM.retrieveOperator(operator.getIdOperateur()).getIdOperateur());
    }
}